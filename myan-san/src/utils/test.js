// calculations.js ဖိုင်မှ function ကို import လုပ်သည်
import { autoCalculateOvernightAndDayOver } from './calculations.js'; 
...
// HTML ထဲသို့ ရလဒ်များကို ရေးထည့်ရန် function
const logToHtml = (message, type = 'default') => {
    const div = document.createElement('div');
    div.textContent = message;
    if (type === 'pass') {
        div.classList.add('test-result', 'pass');
    } else if (type === 'fail') {
        div.classList.add('test-result', 'fail');
    }
...
const testScenarios = [
  ...
  { 
    ...
    start: "2025-8-03T10:00:00", 
    end: "2025-8-05T18:30:00", 
    routeType: "export", 
    cargoType: "normal", 
    cargoLoad: null, 
    expected: { overnight: 1, dayOver: 1 } 
...
function runTests() {
  // အရင်ရလဒ်တွေကို ရှင်းလင်းခြင်း
  outputElement.innerHTML = '';
  summaryElement.textContent = '';
  
  logToHtml("autoCalculateOvernightAndDayOver function အတွက် စမ်းသပ်မှုများ စတင်ပါပြီ...");
  ...
  testScenarios.forEach((scenario) => {
    ...
    const overnightMatch = actual.overnightStayCount === expected.overnight;
    const dayOverMatch = actual.dayOverDelayedCount === expected.dayOver;

    if (overnightMatch && dayOverMatch) {
      logToHtml(`✅ အောင်မြင်: ${description}`, 'pass');
      passedCount++;
    } else {
      allTestsPassed = false;
      logToHtml(`❌ ကျရှုံး: ${description}`, 'fail');
      logToHtml(`   - မျှော်မှန်းထားသော ရလဒ်: အသားအိပ်: ${expected.overnight}, နေ့ကျော်: ${expected.dayOver}`, 'fail-details');
      logToHtml(`   - တကယ်ရရှိသော ရလဒ်:   အသားအိပ်: ${actual.overnightStayCount}, နေ့ကျော်: ${actual.dayOverDelayedCount}`, 'fail-details');
      failedCount++;
    }
  });

  const summaryMessage = `\nစမ်းသပ်မှုပေါင်း ${testScenarios.length} ခုထဲမှ ${passedCount} ခု အောင်မြင်ပြီး ${failedCount} ခု ကျရှုံးခဲ့ပါသည်။`;
...
// Button ကို နှိပ်လိုက်တဲ့အခါ စမ်းသပ်မှုကို စတင်မည်
